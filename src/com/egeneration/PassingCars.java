...
public class PassingCars {
    public static int solution(int[] A) {
        ...
        int resetIteratorValue = 0;
        for (int i = 1; i < A.length; i++) {
            if(firstElementTravelling != A[i]) passingCarCount++;
            else {
                if(!isSimilarToFirstElementTravelling) {
                    resetIteratorValue = i;
                    isSimilarToFirstElementTravelling = true;
                }
            }
            if((i == A.length - 1) && isSimilarToFirstElementTravelling) {
                // reset i value
                i = resetIteratorValue;
                isSimilarToFirstElementTravelling = false;
            }
